...
@section('content')
    <form action="{{ route('admin.user.update', auth()->user()->id) }}" method="POST" onsubmit="return confirm('Do you really want to update your profile?')" class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        @csrf
        @method('PUT')
        <div class="flex flex-col items-center sm:items-start justify-center gap-4 mb-2">
            <div class="relative w-24 h-24 mr-3 rounded-full">
                <img src="{{ asset('img/profile/' . $user->profile_photo) }}" alt="Profile" loading="lazy" class="object-cover w-full h-full rounded-full">
